...
app.post('/webhook', function(request, res) 
{
  ...
  if(intent == "tellmeajoke")
  {
    if(jokeabout == "")
    {
      ...
      {
        var jsonResponse = { "fulfillmentText": joketext };
    ...
    else
    {
      GetJokeAbout(jokeabout, function(joke){
        if(joke == null || joke == "")
        {
          res.json({ "fulfillmentText": "Lo siento creo que no tengo chistes sobre " + jokeabout });
        }
...
function GetJokeAbout(joke, callback)
{
    ...
    Jokes.findOne({ 'text' : { $regex : ".*" + joke + ".*", '$options' : 'i' }},  function (err, joke)
    {
      console.log(joke)
      callback(joke.text);
    });
